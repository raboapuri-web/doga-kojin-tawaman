...
const story=JSON.parse(fs.readFileSync(path.join(root,'src/story-data.json'),'utf8'));
const rows=story.shots.map((s,index)=>({id:s.id,index,narration:s.narration}));
if(rows.length<40) throw new Error(`Expected 40+ beats, got ${rows.length}`);
...
for(const row of rows){
  ...
  q.speedScale=1.07;
  q.pitchScale=-0.03;
  q.intonationScale=0.80;
  q.volumeScale=0.98;
  q.prePhonemeLength=0.09;
  q.postPhonemeLength=0.13;
  ...
  const raw=path.join(tmp,`${row.id}.wav`);
  const padded=path.join(tmp,`${row.id}-pad.wav`);
  fs.writeFileSync(raw,wav);
  execFileSync('ffmpeg',['-y','-loglevel','error','-i',raw,'-af','apad=pad_dur=0.16',padded]);
  ...
  timings.push({id:row.id,index:row.index,start:cursor,end:cursor+duration});
...
fs.writeFileSync(path.join(root,'src/sync-timing.json'),JSON.stringify({durationSeconds:measured+0.3,beats:timings},null,2));
execFileSync('ffmpeg',['-y','-loglevel','error','-i',narrationWav,'-c:a','aac','-b:a','176k','-ar','48000',path.join(audioDir,'narration.m4a')]);
console.log(`Narration duration: ${measured.toFixed(2)}s across ${timings.length} beats`);
