...
const Subtitle=({beat,p}:{beat:Beat;p:number})=>{
  ...
  return <>
    <div style={{position:'absolute',left:70,top:54,color:'rgba(232,226,215,.64)',fontFamily:'Noto Sans CJK JP, sans-serif',fontSize:20,fontWeight:700,letterSpacing:3}}>
      {beat.chapter} / {beat.id}
    </div>
    <div style={{position:'absolute',left:140,right:140,bottom:60,display:'flex',justifyContent:'center',pointerEvents:'none'}}>
      <div style={{maxWidth:1520,padding:'16px 34px 19px',borderRadius:12,background:'rgba(5,7,11,.62)',backdropFilter:'blur(8px)',color:'#f1ece2',fontFamily:'Noto Sans CJK JP, sans-serif',fontSize:40,fontWeight:800,lineHeight:1.48,textAlign:'center',letterSpacing:.5,textShadow:'0 3px 18px #000',opacity}}>
        {chunks[i]}
      </div>
    </div>
  </>;
...
const Film=()=>{
  ...
  const ts=timing.beats.length?timing.beats:script.beats.map((b,i)=>({id:b.id,index:i,start:i*14,end:(i+1)*14}));
  ...
  return <AbsoluteFill style={{background:'#07090d',overflow:'hidden'}}>
    ...
    <div style={{position:'absolute',inset:0,background:'radial-gradient(ellipse at 50% 46%,transparent 35%,rgba(0,0,0,.18) 70%,rgba(0,0,0,.7) 100%)',pointerEvents:'none'}}/>
    <Subtitle beat={beat} p={p}/>
  </AbsoluteFill>;
};
const Root=()=> <Composition id="V20MinatoHappiness" component={Film} width={1920} height={1080} fps={30} durationInFrames={Math.max(1,Math.ceil(timing.durationSeconds*30))}/>;
registerRoot(Root);
