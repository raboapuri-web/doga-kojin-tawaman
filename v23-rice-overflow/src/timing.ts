...
type Sync={durationSeconds:number;beats:BeatTiming[]};
const measured=sync as Sync;
const scriptBeats=scriptData.beats;

const fallback=():Sync=>{
  const secondsPerBeat=9;
  return {
    durationSeconds:scriptBeats.length*secondsPerBeat+0.5,
    beats:scriptBeats.map((b,index)=>({id:b.id,index,start:index*secondsPerBeat,end:(index+1)*secondsPerBeat})),
  };
};

export const syncTiming:Sync=measured.beats.length===scriptBeats.length?measured:fallback();
