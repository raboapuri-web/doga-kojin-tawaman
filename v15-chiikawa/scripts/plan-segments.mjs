...
const fps=30;
const targetSeconds=175;
const maxSeconds=225;
const segments=[];
let startFrame=0;
...
if(lastEndFrame<Math.ceil(sync.durationSeconds*fps)-1){
  segments.push({id:String(segments.length).padStart(2,'0'),start:lastEndFrame+1,end:Math.ceil(sync.durationSeconds*fps)-1});
}
