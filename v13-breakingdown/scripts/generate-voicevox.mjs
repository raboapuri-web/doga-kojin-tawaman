...
for(let i=0;i<script.beats.length;i++){
  ...
  const q=await fetch(`${base}/audio_query?speaker=${style.id}&text=${encodeURIComponent(beat.narration)}`,{method:'POST'}).then(r=>r.json());
  q.speedScale=speed;
  ...
  const raw=path.join(tmp,`beat-${String(i).padStart(2,'0')}.wav`);
  const padded=path.join(tmp,`beat-${String(i).padStart(2,'0')}-pad.wav`);
  fs.writeFileSync(raw,wav);
  execFileSync('ffmpeg',['-y','-loglevel','error','-i',raw,'-af','apad=pad_dur=0.12',padded]);
  ...
  cursor+=duration; files.push(padded);
...
execFileSync('ffmpeg',['-y','-loglevel','error','-f','concat','-safe','0','-i',list,'-c','copy',path.join(audioDir,'narration.wav')]);
const measured=Number(execFileSync('ffprobe',['-v','error','-show_entries','format=duration','-of','default=nw=1:nk=1',path.join(audioDir,'narration.wav')],{encoding:'utf8'}).trim());
const sync={durationSeconds:measured+0.25,beats:timings};
fs.writeFileSync(path.join(root,'src/sync-timing.json'),JSON.stringify(sync,null,2));
console.log(`Narration duration: ${measured.toFixed(2)}s`);
