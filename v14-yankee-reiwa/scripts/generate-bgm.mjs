...
execFileSync('ffmpeg',[
  '-y','-loglevel','error',
  '-f','lavfi','-i',`sine=frequency=48:duration=${d}`,
  '-f','lavfi','-i',`sine=frequency=72:duration=${d}`,
  '-f','lavfi','-i',`anoisesrc=color=brown:duration=${d}:amplitude=0.018`,
  '-filter_complex',
  `[0:a]volume=0.010,lowpass=f=180[a0];`+
  `[1:a]volume=0.004,lowpass=f=240[a1];`+
  `[2:a]lowpass=f=520,highpass=f=45,volume=0.38[a2];`+
...
console.log(`BGM ready: ${d}s (no SFX)`);
