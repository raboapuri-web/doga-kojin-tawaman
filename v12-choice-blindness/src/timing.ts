...
export const getActiveBeatAtSeconds=(seconds:number)=>{
  ...
  let beat=beats.find((b)=>seconds>=b.start&&seconds<b.end)??beats[beats.length-1];
  const duration=Math.max(.001,beat.end-beat.start);
  const progress=Math.max(0,Math.min(1,(seconds-beat.start)/duration));
  return {index:beat.index,id:beat.id,progress,speechProgress:progress,actualStartSeconds:beat.start,actualEndSeconds:beat.end};
};
